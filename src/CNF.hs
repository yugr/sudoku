module CNF (Lit (..), Clause, CNF.CNF, lit, notlit, CNF.or, get_var, neqs, nvars, print_dimacs, solve, is_negation) where
...
nvars :: CNF -> Int
nvars cnf =
  -- Be careful to avoid creating intermediate lists
  let
    nvars_clause acc = Data.List.foldl' (\acc l -> max acc (get_var l)) acc
  in
    Data.List.foldl' nvars_clause 0 cnf
